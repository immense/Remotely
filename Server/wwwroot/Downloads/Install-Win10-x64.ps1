...
function Install-Remotely {
	...
	if ($ArgList.Contains("-path")) {
		...
		Copy-Item -Path $ArgList[$SourceIndex] -Destination "$InstallPath\Remotely-Win10-x64.zip"

	}
	else {
		$ProgressPreference = 'SilentlyContinue'
		Write-Log "Downloading client..."
		Invoke-WebRequest -Uri "$HostName/Downloads/Remotely-Win10-x64.zip" -OutFile "$InstallPath\Remotely-Win10-x64.zip" 
		$ProgressPreference = 'Continue'
	}

	Expand-Archive -Path "$InstallPath\Remotely-Win10-x64.zip" -DestinationPath "$InstallPath"  -Force
...
function Install-DesktopRuntime() {
	...
	if ($RuntimeRegKey -eq $null) {
		...
		$Response = Invoke-WebRequest -Uri "https://dotnet.microsoft.com/download/dotnet-core/thank-you/runtime-desktop-3.1.1-windows-x64-installer"
