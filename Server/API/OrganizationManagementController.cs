...
namespace Remotely.Server.API
{
    ...
    public class OrganizationManagementController : ControllerBase
    {
        ...
        [HttpPost()]
        [ServiceFilter(typeof(ApiAuthorizationFilter))]
        public async Task<IActionResult> CreateOrganization([FromBody] OrganizationViewModel vm)
        {
            if (User.Identity.IsAuthenticated &&
                !DataService.GetUserByNameWithOrg(User.Identity.Name).IsAdministrator)
            {
            ...
            var newOrg = await DataService.CreateOrganization(vm.OrganizationName);
            return Ok(newOrg.ID);
        }

        [HttpPost("ApiToken")]
        [ServiceFilter(typeof(ApiAuthorizationFilter))]
        public async Task<IActionResult> CreateApiToken([FromBody] ApiTokenModel vm)
        {
            if (User.Identity.IsAuthenticated &&
                !DataService.GetUserByNameWithOrg(User.Identity.Name).IsAdministrator)
            {
